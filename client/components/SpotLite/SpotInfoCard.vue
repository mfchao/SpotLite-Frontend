...
onBeforeMount(async () => {
  ...
  const fetchedUser = await getUsers(props.user.username);
  if (fetchedUser[0]) {
...
<template>
  <div class="card" v-if="loaded">
    <img class="profile" :src="userPhoto" alt="User profile picture" v-if="userPhoto != ''"/>
    <p class="username">{{ props.user.username }}</p>
    <p class="bio">{{ props.user.bio }}</p>
    <div class="socials">
      <a :href="props.user.socials" target="_blank">{{props.user.socials }}</a>
    </div>
  </div>
</template>
